...
static void AddCommentsToFile(const filesystem::path &path, const string &comment, const ProgramOptions &options)
{
  ...
  if (itFreeComment != PvsStudioFreeComments::Comments.end())
  {
    ...
    if (needToChangeComment || options.m_removeLines)
    {
      source.erase(source.begin() + distance(source.c_str(), beg), source.begin() + distance(source.c_str(), end));
      WriteComment(path, comment, source, isUnixLineEnding, enc, bomLen, options);
    }
  }
  else
  {
...
Usage:
  ...
  how-to-use-pvs-studio-free -c <1|2|3> [-m] [-s] [-h] [-r] <strings> ...
...
Options:
  ...
  -r, /r, --remove
    Remove comments done by this tool.
...
int main(int argc, const char *argv[])
{
  ...
  ParsedOptions options = {
    ...
    { { "-r", "/r", "--remove" },    false,  [&](string &&)    { progOptions.m_removeLines  = true; } },
  };
